...
public class BottomViewOfBinaryTree {
    ...
    private static ArrayList<Integer> bottomView(Node root) {
        ...
        for (int i = 0; i < len; i++)
            ans.add(null);
        Queue<TopViewOfBinaryTree.Pair> queue = new ArrayDeque<>();
        queue.add(new TopViewOfBinaryTree.Pair(root, Math.abs(minMax[0])));
        while (!queue.isEmpty()) {
            int size = queue.size();
            for (int i = 0; i < size; i++) {
                TopViewOfBinaryTree.Pair rem = queue.remove();
                Node remnode = rem.node;
                int remlevel = rem.hlevel;
                ans.set(remlevel, remnode.val);
                if (remnode.left != null)
                    queue.add(new TopViewOfBinaryTree.Pair(remnode.left, remlevel - 1));
                if (remnode.right != null)
                    queue.add(new TopViewOfBinaryTree.Pair(remnode.right, remlevel + 1));
            }
        }
        return ans;
    }
